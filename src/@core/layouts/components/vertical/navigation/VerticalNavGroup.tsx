...
const VerticalNavGroup = (props: Props) => {
  ...
  const conditionalTextColor = (param: boolean) => {
    if(param){
      return {
  ...
  const conditionalBgColor = () => {
    if (theme.palette.mode === 'dark') {
      return {
        ...
        '&.Mui-selected': {
          backgroundColor: `${hexToRGBA(theme.palette.primary.dark, 0.9)}`,
          '&:hover': {
            backgroundColor: `${hexToRGBA(theme.palette.primary.dark, 0.8)}`
          }
        }
      }
        ...
        >
          <ListItemButton
            ...
            sx={{
              ...
              width: '100%',
              ...conditionalBgColor(),
              boxShadow: groupActive.includes(item.title) || currentActiveGroup.includes(item.title) ? theme.shadows[3] : 0,
              borderRadius:1,
          ...
          >
            {isSubToSub ? null : (
              <ListItemIcon
                sx={{
                  ...conditionalTextColor(groupActive.includes(item.title) || currentActiveGroup.includes(item.title)),
              ...
              >
                <UserIcon icon={IconTag as string} {...(parent && { fontSize: '0.875rem' })} />
              </ListItemIcon>
            )}
            <MenuItemTextWrapper sx={{ ...menuGroupCollapsedStyles, ...(isSubToSub ? { ml: 9 } : {}) }}>
              ...
              <Box className='menu-item-meta' sx={{
                ml: 0.8,
                display: 'flex',
                alignItems: 'center',
                '& svg': {
