...
export const formatMonthShort = (value: Date | string, digits: '2-digit' | 'short') => {
  let date = new Date()

  if (value instanceof Date) {
    date = new Date(value)

    const formatting: Intl.DateTimeFormatOptions = { month: digits }

    return new Intl.DateTimeFormat('pt-BR', formatting).format(date)
  } else {
    const str = value.slice(0, 3)
    const month = str.charAt(0).toUpperCase()

    return month + str.slice(1)
  }
}

export const formatStringWithDots = (value: string, dotsNumber: number) => {
  let descrition = ''
  let dots = ''
  if(value?.length > dotsNumber) dots = '...'

  for (let index = 0; index < dotsNumber; index++) {
    descrition = descrition + (value[index] != undefined ? value[index] : '')
  }
