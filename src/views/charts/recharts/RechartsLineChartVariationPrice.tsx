          ...
          payload.map((i: any) => {
            return (
              <Box sx={{ display: 'flex', alignItems: 'center', '& svg': { color: i.fill, mr: 2.5 } }} key={i.dataKey}>
                {i.dataKey == 'productPrice' ? (
                  <Typography sx={{ fontSize: '0.875rem' }}>{`Valor: ${i.payload['priceCurrency']}`}</Typography>
                ) : (
                  <Typography
...
const PriceVariationLineChart = (props: Props) => {
        ...
        <Box sx={{ height: 350 }}>
          <ResponsiveContainer>
            <LineChart height={350} data={data} style={{ direction: 'ltr' }} margin={{ left: 15, right: 15 }}>
              ...
              <Line dataKey='productPrice' stroke='#E09E00' strokeWidth={3} />
            </LineChart>
          </ResponsiveContainer>
        </Box>
      </CardContent>
...
export default PriceVariationLineChart
